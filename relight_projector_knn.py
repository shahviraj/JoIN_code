...
def project(
    ...
    if not opts.sh_fixed:
        ...
        final_sh_image = gan_to_final(synth_sh_image)
    ...
    if not opts.al_fixed:
        ...
        final_al_image = gan_to_final(synth_al_image)
    ...
    if opts.use_specular:
        ...
        final_sp_image = gan_to_final(synth_sp_image)
...
def run_metric(fim, fsh, fal, fsp, tarim, tarsh, taral, tarsp, opts):
    ...
    for j, met in enumerate(metriclist):
        if os.path.exists(opts.outdir + f'/{met}_metric.pt'):
            metric_dict = torch.load(opts.outdir + f'/{met}_metric.pt')
        else:
            metric_dict = {}
        ...
        torch.save(metric_dict, opts.outdir + f'/{met}_metric.pt')
